...
| `/sessions/[threadId]/replay` | client → `/api/session-replay` | Conversation replay — the thread acted out on one scrubable clock: a lane per agent (sub-agents nested), step log following the playhead, containers drawn as hollow envelopes. |
| `/sessions/[threadId]/fleet` | client → `/api/session-replay` | Conv fleet — the same script as a pixel office: a desk per agent, skills at the library, tools at the wall, delegations walking over with speech bubbles, thought clouds while thinking; click a character for its personnel file (declared agent-definition + observed models/tools), or a book on the library / a tool on the wall for its card (declared description, who used it, last result). |
