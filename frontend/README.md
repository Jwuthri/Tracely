...
- **`trace-table/` holds the parts that aren't about tables:** `content.tsx` (how a payload becomes readable — chat transcripts, multimodal blocks, tool calls, state writes, and the badges that label them; it knows PROVIDER SHAPES, touches none of the table's state, and is tested against real wire payloads in `content.test.tsx`), `format.ts` (pure text/number helpers), `columns.ts`, `icons.tsx`.
