...
- **Column groups** with level badges and subtle group dividers: **C** (conversation: title, summary, usage), **M** (message/turn: role, #, time, content, usage), **S** (step/span: #, type, time, duration, agent, model, thinking, name, input, output, usage). Depth-coloured left borders (C=blue, M=green, S=purple).
- **Rich cells:** chat transcripts → a compact "conversation" pill that opens a floating bubble panel; structured data (tool args/results, output schemas, completion objects) → a `{ }`/`[ ]` JSON pill with a syntax-highlighted panel; multimodal message content → text + image-thumbnail / file chips; per-level **usage** → a Σ pill with a Tokens/Cost breakdown. `THINKING` is a first-class span type (its reasoning shows in the Thinking column). Floating panels use `createPortal` to escape the table's overflow.
...
- **Controls:** Expand/Collapse All (cascades to the step level), a Columns visibility menu and an Enlarge (full-width breakout) toggle — both **persisted to `localStorage`**. Whole-row click opens the trace.
