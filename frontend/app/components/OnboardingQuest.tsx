...
/* The gamified onboarding quest — a floating launcher (bottom-right, progress ring) opening a
   ...
   → traces / evaluators / trends / fleet → failure → case → gate. Counts come from
   /api/onboarding; page-visit steps tick via usePathname. Mounted once in the (app) layout, so
   it survives navigation and sees every route. */
    ...
    <li>
      <button type="button" onClick={onToggle} className="flex w-full items-center gap-2.5 px-4 py-2 text-left">
        ...
        <span className="shrink-0 font-mono text-[10px] text-fg-faint">
          {step.skipped && !step.done ? "skipped" : `+${XP_PER_STEP} xp`}
        </span>
      </button>
      {open && (
        <div className="px-4 pb-3 pl-[42px]">
          <p className="text-[12px] leading-relaxed text-fg-muted">{step.detail}</p>
          <div className="mt-2 flex flex-wrap items-center gap-2">
            <a href={step.href} className="btn-ghost">
              {step.cta} →
            </a>
            {onCopyKey && (
              <button type="button" onClick={onCopyKey} className="btn-ghost">
                Copy key
              </button>
            )}
            {onSkipLlm && !complete && (
              <button type="button" onClick={onSkipLlm} className="btn-ghost">
...
export function OnboardingQuest() {
  ...
  // tick page-visit steps as the user moves through the app
  useEffect(() => {
    const m = visitMarker(pathname ?? "");
    if (!m) return;
    setLocal((l) => (!l || l.visited.includes(m) ? l : { ...l, visited: [...l.visited, m] }));
  }, [pathname]);
  ...
  const expanded = openStep ?? firstUndone;
  ...
  return (
    <>
      ...
      {open && (
        <div className="fixed bottom-20 right-5 z-40 flex max-h-[min(640px,calc(100vh-120px))] w-[360px] flex-col overflow-hidden rounded-xl border border-line bg-ink-900 shadow-2xl">
          <div className="border-b border-line px-4 py-3">
            ...
            <div className="mt-2 flex items-center justify-between font-mono text-[10.5px] text-fg-faint">
              <span className={allDone ? "text-ok" : "text-signal"}>
                {complete * XP_PER_STEP} xp · {questRank(complete, total)}
              </span>
              <span>
                {complete} / {total}
              </span>
            ...
            {allDone ? (
              <div className="px-4 py-8 text-center">
                ...
                <p className="mt-1.5 text-[12px] text-fg-muted">
                  From here it runs itself: every production trace is graded, failures cluster, and your gates
                  hold the line.
                </p>
              </div>
            ) : (
                    ...
                    {steps
                      ...
                      .map((s) => (
                        <StepRow
                          ...
                          step={s}
                          open={expanded === s.id}
                          onToggle={() => setOpenStep(expanded === s.id ? "" : s.id)}
                          onCopyKey={s.id === "key" ? copyKey : null}
                          onSkipLlm={
                            s.id === "llm" ? () => setLocal((l) => (l ? { ...l, llm_skipped: true } : l)) : null
                          }
                        />
                      ))}
                  </ol>
