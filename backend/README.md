...
| `routers/assistant.py` | The dashboard's chat widget: conversations, one turn per POST (`assistant_service.answer`) on the server's own key, and attachments. |
...
| `POST /api/assistant/chat` | `assistant.py` | one turn of the in-app chat widget: `{message, chat_id?, attachments?, path}` → `{chat_id, title, reply}`, or `{disabled: true}` when the deployment has no assistant model configured. Runs on **Tracely's own** OpenRouter key (`provider.use_server_key`, `ASSISTANT_MODEL`, default `google/gemini-3.7-flash` at `medium` reasoning) — the one LLM call a customer doesn't pay for, so it works in a workspace that brought no key. The **server** owns the transcript (`assistant_chats`), so the conversation survives a new browser; a failed turn is not stored. Each reply logs `assistant_usage` with the project id, so our spend is attributable. |
