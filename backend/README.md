...
Execution modes: `batch` (independent, default) or `sequential` (the items are graded as ONE durable conversation with the judge — rubric as system prompt, then item → verdict → item → verdict, persisted via LangGraph's Postgres checkpointer so only the new item goes over the wire. A step judge's conversation spans the steps of one message; a message judge's spans the turns of one thread. Each pass resets and rebuilds its conversation from item 1; the previous turn's result of the same metric is seeded into the first item, and without a reachable checkpointer the run-so-far is pasted into each prompt instead).
