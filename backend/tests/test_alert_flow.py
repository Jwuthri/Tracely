...
# ── the rule assistant ────────────────────────────────────────────────────────
# The model is stubbed: what needs pinning is the SHAPING — a sentence becomes a condition of the
# right shape for its trigger family, and a flow the canvas can render.


def _stub_model(monkeypatch, draft):
    from tracely.infrastructure.llm import provider
    from tracely.services import alert_assistant_service as assistant

    monkeypatch.setattr(provider, "llm_enabled", lambda: True)
    monkeypatch.setattr(provider, "run_structured_agent", lambda *a, **k: draft)
    return assistant


def test_assistant_shapes_an_event_condition(monkeypatch):
    from tracely.services.alert_assistant_service import GeneratedRule

    draft = GeneratedRule(
        name="PII on support-bot",
        trigger="trace_failed",
        target_agent="support-bot",
        contains="pii",
        score_name="tracely.run.grounded",
        env="ignored-for-this-trigger",
        threshold=0.9,
        steps=[{"name": "Post to Slack", "step_type": "slack", "config": {"url": "", "text_template": "x"}}],
        message="Paste your Slack URL.",
    )
    assistant = _stub_model(monkeypatch, draft)
    out = assistant.generate_rule("p1", "slack me about pii")
    # An event condition carries filters only — no threshold, and no `env` on a trace trigger.
    assert out["condition"] == {
        "type": "trace_failed",
        "contains": "pii",
        "score_name": "tracely.run.grounded",
    }
    assert out["target_agent"] == "support-bot"
    assert [s["step_type"] for s in out["steps"]] == ["slack"]


def test_assistant_shapes_a_threshold_condition_and_wires_a_chain(monkeypatch):
    from tracely.domain.alerting import TRIGGER_NODE_ID
    from tracely.services.alert_assistant_service import GeneratedRule

    draft = GeneratedRule(
        name="Quality slipping",
        ...
        min_samples=15,
        steps=[
            {"name": "Write a note", "step_type": "llm_prompt", "config": {"user_prompt_template": "why?"}},
            {"name": "Post it", "step_type": "slack", "config": {"url": "", "text_template": "{{ steps[0].result.text }}"}},
        ],
        message="ok",
    )
    assistant = _stub_model(monkeypatch, draft)
    out = assistant.generate_rule("p1", "tell me when quality slips")
    assert out["condition"] == {
        "type": "fail_rate_over",
        "threshold": 0.25,
        "window_minutes": 30,
    ...
    # The generated flow arrives WIRED from the trigger, or the canvas would show parked steps that
    # never run.
    edges = out["flow_layout"]["edges"]
    assert edges[0]["source"] == TRIGGER_NODE_ID
    assert [e["target"] for e in edges] == [s["id"] for s in out["steps"]]


def test_assistant_drops_a_step_type_the_engine_cannot_run(monkeypatch):
    from tracely.services.alert_assistant_service import GeneratedRule

    draft = GeneratedRule(
        name="x", trigger="cluster_new", steps=[{"name": "?", "step_type": "slack", "config": {}}], message=""
    )
    assistant = _stub_model(monkeypatch, draft)
    # Force an unknown type past the schema the way a future model version could.
    draft.steps[0].step_type = "teleport"  # type: ignore[assignment]
    out = assistant.generate_rule("p1", "anything")
    assert out["steps"] == []


def test_assistant_without_a_workspace_key_says_so(monkeypatch):
    from tracely.infrastructure.llm import provider
    from tracely.services import alert_assistant_service as assistant

    monkeypatch.setattr(provider, "llm_enabled", lambda: False)
    with pytest.raises(RuntimeError, match="OpenRouter"):
        assistant.generate_rule("p1", "draft me something")
