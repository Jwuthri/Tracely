...
def gen(agent: str, messages, output, in_tok: int, out_tok: int, *, model: str = "gpt-4o",
    ...
    with tracely.llm(model, agent=agent, temperature=temperature, top_p=top_p, max_tokens=max_tokens,
                     frequency_penalty=0.0, presence_penalty=0.0, seed=7,
                     tool_calls=tool_calls, metadata=meta) as g:
...
def turn_io(span, user, assistant) -> None:
    ...
    tracely.set_io(span, input={"role": "user", "content": u}, output={"role": "assistant", "content": a})
...
def seed_rag():
    ...
    with tracely.agent(SUPPORT, version="v4", conversation=conv, turn=0,
                       user="u_7741", trace_name="docs Q&A (RAG)") as a:
        u = "How do I rotate my API key without downtime?"
        ans = ("Create a second key in **Settings → API keys**, deploy it, then revoke the old one — "
               "both are valid during the overlap, so there's zero downtime.")
        turn_io(a, u, ans)
        guard("input_guardrail", SUPPORT, u, {"action": "allow", "flags": [], "pii": False}, policy="safety-v2")
        with tracely.chain("rag_pipeline", agent=SUPPORT):  # groups the retrieval sub-steps
            ...
            retrieve("search_docs", SUPPORT,
                     {"query": u, "top_k": 3, "index": "help-center"},
                     {"hits": [
                         {"id": "doc/api-keys", "score": 0.92, "title": "Rotating API keys"},
                         {"id": "doc/zero-downtime", "score": 0.81, "title": "Zero-downtime deploys"},
                         {"id": "doc/auth", "score": 0.74, "title": "Authentication overview"}]},
                     vector_store="pgvector")
            time.sleep(0.2)
        gen(SUPPORT, [{"role": "system", "content": "Answer ONLY from the retrieved docs. Cite them."},
                      {"role": "user", "content": u}], ans, 760, 88, think_tok=40,
...
def seed_laptop():
    ...
    with tracely.agent(SHOP, version="v3", conversation=conv, turn=0,
                       user="u_3310", trace_name="laptop recommendation") as a:
        ...
        gen(SHOP, sys_user("Classify the shopper's intent into the schema.", u0),
            ...
            96, 44, model="gpt-4o-mini", temperature=0.0, metadata={"task": "intent_classification"})
...
def seed_order_issue():
    ...
    with tracely.agent("router", version="v2", role="orchestrator", conversation=conv, turn=0,
                       user="u_9920", trace_name="order issue (multi-agent)") as root:
        ...
        with tracely.agent("shipping-agent", role="specialist", conversation=conv, handoff_from="router"):
        ...
        with tracely.agent("billing-agent", role="specialist", conversation=conv, handoff_from="router"):
...
def seed_multimodal():
    conv = "conv-" + uuid.uuid4().hex[:8]
    with tracely.agent(SUPPORT, version="v4", conversation=conv, turn=0,
                       user="u_4471", trace_name="damaged item return") as a:
        user_msg = as_content(
...
def seed_attachments():
    ...
    with tracely.agent(SUPPORT, version="v4", conversation=conv, turn=0,
                       user="u_5582", trace_name="warranty claim") as a:
        user_msg = as_content(
...
def seed_research():
    conv = "conv-" + uuid.uuid4().hex[:8]
    with tracely.agent(RESEARCH, version="v1", conversation=conv, turn=0,
                       user="u_2027", trace_name="competitive research") as a:
        u = "Summarise how our top 3 competitors price their enterprise tier, with sources."
        ans = ("**Pricing summary (enterprise):** Competitor A is seat-based (~$60/seat/mo, annual), "
        ...
        turn_io(a, u, ans)
        think(RESEARCH, "Plan: search each competitor's pricing page, retrieve the relevant sections, "
                        "normalise to per-month, then synthesise with citations.", 180, model="claude-3-5-sonnet")
        for comp in ("Competitor A", "Competitor B", "Competitor C"):
            retrieve("web_search", RESEARCH,
                     {"query": f"{comp} enterprise pricing", "engine": "tavily"},
                     {"hits": [{"url": f"https://{comp.split()[-1].lower()}.example/pricing", "score": 0.88}]},
                     competitor=comp)
        gen(RESEARCH, sys_user("Synthesise the pricing research into a sourced summary.", u), ans,
            3200, 540, model="claude-3-5-sonnet", think_tok=180, max_tokens=2048,
...
def seed_guardrail_block():
    conv = "conv-" + uuid.uuid4().hex[:8]
    with tracely.agent(SUPPORT, version="v4", conversation=conv, turn=0,
                       user="u_0001", trace_name="guardrail block") as a:
        ...
        turn_io(a, u, ans)
        guard("input_guardrail", SUPPORT, u,
              {"action": "block", "flags": ["prompt_injection", "data_exfiltration"], "severity": "high"},
              policy="safety-v2")
        gen(SUPPORT, sys_user("If the guardrail blocked, refuse safely and offer a benign alternative.", u),
            ans, 120, 36, temperature=0.2)
...
def seed_tool_recovery():
    conv = "conv-" + uuid.uuid4().hex[:8]
    with tracely.agent(SHOP, version="v3", conversation=conv, turn=0,
                       user="u_6610", trace_name="address lookup (with retry)") as a:
        ...
        turn_io(a, u, ans)
        use_tool("geocode_address", SHOP, {"provider": "primary", "q": "work address"},
        ...
        use_tool("geocode_address", SHOP, {"provider": "backup", "q": "work address"},
                 {"line1": "500 Howard St", "city": "San Francisco", "state": "CA", "zip": "94105"})
        use_tool("update_shipping_address", SHOP, {"order_id": "ORD-5582", "address_id": "addr_work"},
                 {"ok": True})
...
def seed_hallucination():
    conv = "conv-" + uuid.uuid4().hex[:8]
    with tracely.agent(SHOP, version="v3", conversation=conv, turn=0,
                       user="u_3310", trace_name="stock check") as a:
...
def seed_missing_tool():
    conv = "conv-" + uuid.uuid4().hex[:8]
    with tracely.agent(SUPPORT, version="v4", conversation=conv, turn=0,
                       user="u_1240", trace_name="account balance") as a:
...
def seed_faq():
    conv = "conv-" + uuid.uuid4().hex[:8]
    with tracely.agent(SUPPORT, version="v4", conversation=conv, turn=0,
                       user="u_0420", trace_name="support hours FAQ") as a:
        u = "What are your support hours?"
        ans = "We're here 24/7 via chat, and 8 am-8 pm ET by phone."
        turn_io(a, u, ans)
        gen(SUPPORT, sys_user("Answer the FAQ.", u), ans, 90, 28, model="gpt-4o-mini", temperature=0.0)
