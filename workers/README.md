...
| `evaluate_run_task` | after ingest, per trace | run the project's evaluators (targeting + sampling) → write `scores` → cheap structural failure clustering → fold the turn into the thread's rolling summary (best-effort). |
